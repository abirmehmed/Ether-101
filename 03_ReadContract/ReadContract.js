...
// Declare the rules for read-only contracts:
// The parameters are contract address `address`, contract ABI `abi`, Provider variable `provider`
...
// The first way to input abi: copy the full text of abi
...
// The second way to input abi: enter the functions that the program needs to use, separated by commas, ethers will automatically help you convert them to the corresponding abi
...
const main = async () => {
    ...
    console.log(`Vitalik's position: ${ethers.formatEther(balanceWETH)}\n`)
    ...
    console.log(`Vitalik's position: ${ethers.formatEther(balanceDAI)}\n`)
}
