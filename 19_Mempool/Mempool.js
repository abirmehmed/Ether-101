...
// 1. Create provider and wallet, use wss connection for event listening instead of http
...
console.log("\n2. Limit the rate of calling rpc interface")
// 2. Limit the access rate of rpc, otherwise the call frequency will exceed the limit and an error will be reported.
...
const main = async () => {
    ...
    provider.on("pending", async (txHash) => {
        if (txHash && i < 100) {
            // Print txHash
            console.log(`[${(new Date).toLocaleTimeString()}] Listen for Pending transactions ${i}: ${txHash} \r`);
            i++
            }
    });
    ...
    provider.on("pending", throttle(async (txHash) => {
        if (txHash && j <= 100) {
            ...
            console.log(`\n[${(new Date).toLocaleTimeString()}] Listen for Pending transactions ${j}: ${txHash} \r`);
            console.log(tx);
            j++
            }
    }, 1000));
